...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:  # Check for mouse button press
            ...
            if bg == menu:
                ...
                message_displayed = True
                #ememies guess for this turn is gnerated randomly
                computerXGuess = random.randint(1,5)
                computerYGuess = random.randint(1,5)
                
                #checks if the computer guess the x co-ord of any player spot before then checking the y co-ord
                if computerXGuess == playerX1:
                        if computerYGuess == playerY1:
                            #if computer is correct increment its score and reset the players object so it cant be scored again
                            enemyHitCount += 1
                            playerX1 = 0
                            playerY1 = 0
                            #checks if the enemy has hit all 5 pieces
                            if enemyHitCount == 5:
                                computerWin = True
                                
                            else:
                                #creates a variable to check if the enemy has hit a piece this turn and resets to player turn
    ...
    # display logic for ballThrowing screen
        ...
        if message_displayed:
            if PlayerWin == True:
                window.blit(friendlyWin, (width // 2 - friendlyWin.get_width() // 2, height // 3 - friendlyWin.get_height() //2 ))
                window.blit(friendlyWin, (width // 2 - friendlyWin.get_width() // 2, height// 3 + friendlyWin.get_height()))
            elif playerXCord == objectXCord and objectYCord == landing_segment:
    ...
    #meant to display the results of the enemies turn            
    elif bg == enemyresult:
        if message_displayed:
            if computerWin == True:
